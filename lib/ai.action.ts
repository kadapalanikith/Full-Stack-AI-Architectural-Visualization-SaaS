...
/**
 * Fetches an image from the given URL and returns it as a Base64-encoded Data URL.
 *
...
export const generate3DView = async ({ sourceImage }: Generate3DViewParams): Promise<Generate3DViewResult> => {
    ...
    if (!rawImageUrl) return { renderedImage: null, rendered: undefined };

    const renderedImage = rawImageUrl.startsWith('data:')
        ? rawImageUrl
        : await fetchAsDataUrl(rawImageUrl);

    return { renderedImage, rendered: response };
};